...
- [x] **CHANGELOG.md** - 已更新到v1.7.6
  - 添加了v1.7.6版本记录（API参数优化、Tesseract兜底、关键词修复）
  - 包含完整版本历史
...
- [x] **pdf_processor.py**
  ...
  - ✅ 多OCR引擎支持 (LocalPaddleOCR, BaiduPaddleOCR, AliyunOCR, DeepSeekOCR)
  - ✅ OCR引擎自动选择 (环境变量检测)
...
### 2. OCR引擎配置
- 支持4种OCR引擎：本地PaddleOCR-VL、百度飞桨PaddleOCR-VL-1.5、阿里云OCR、SiliconFlow API
- 百度飞桨默认使用 VL-1.5 端点，只需设置 TOKEN
- 环境变量自动检测和优先级选择
- 文档说明与代码实现完全一致 ✅
