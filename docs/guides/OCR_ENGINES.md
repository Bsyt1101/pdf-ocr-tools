...
本工具支持两种调用模式，根据是否设置 `BAIDU_PADDLEOCR_URL` 自动切换：

- **未设置 `BAIDU_PADDLEOCR_URL`**（默认）→ 使用**异步公共 API**（无需专属 URL）
- **设置了 `BAIDU_PADDLEOCR_URL`** → 使用**同步专属 API**（响应更快）

| | 同步解析（专属 URL） | 异步解析（默认） |
|--|---------|---------|
| 调用方式 | POST 发送图片，等待返回 | 提交任务，轮询查询状态 |
| 适合场景 | 单页图片，实时处理 | 无需专属 URL，通用 |
| 认证方式 | `token {TOKEN}` | `bearer {TOKEN}` |
| URL 要求 | 需要用户专属 URL | 使用公共端点 |
